...
import { HevyApiError, HevyClient } from "../../src/lib/client";
import { mockFetchError, mockFetchSuccess } from "../setup";
...
describe("HevyClient - API Contract Compliance", () => {
	...
	describe("POST /v1/routine_folders - createRoutineFolder()", () => {
		...
		it("should return 201 on successful creation (per spec line 897)", async () => {
			...
			const result = await client.createRoutineFolder({
				routine_folder: { title: "New Folder" },
			});
				...
				const workout = {
					workout: {
						...
						exercises: [
							{
								exercise_template_id: "D04AC939",
								sets: [{ type: "normal", weight_kg: 100, reps: 10, rpe: null }],
							},
						],
					},
				};
	...
	describe("HevyApiError", () => {
		it("should create error with status and data", () => {
			const error = new HevyApiError("Test error", 400, {
				detail: "Bad request",
			});
